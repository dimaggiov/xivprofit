...
import { listPoeticItems } from '../graphql/queries'
...
async function getPoeticsData() {
    try {
        const data = await API.graphql(graphqlOperation(listPoeticItems))
        return data
    }
    catch (err) {
    ...
    switch (collumn) {
        case 0: //sort by name
            if (tableSortOrder == 'asc') {
                tableSortOrder = 'dsc'
                secondaryTomeData.data.listPoeticItems['items'].sort((a, b) => {
                    if (a['name'] > b['name']) {
                        return 1;
                    }
                    if (a['name'] < b['name']) {
...
function buildTable() {
    var tableOfItems = ''
    secondaryTomeData.data['listPoeticItems']['items'].forEach(element => {
